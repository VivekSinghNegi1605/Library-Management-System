...
@admin_view.route('/users/delete/<int:user_id>', methods=['GET']) #this one has an error revisit
@admin_manager.admin.login_required
def user_delete(user_id):
	id = int(user_id)

	if id is not None:
		user_manager.user_delete(id)
...
def book_add():
	...
	if request.method == 'POST':
		...
		available = request.form['avaliable']
		description = request.form['desc']
		edition = request.form['edition']

		new_book = book_manager.add(title=title, quantity=quantity, available=available, description=description, edition=edition)


		return render_template('books/add.html', msg = "Book has been successfully added!")
...
def book_edit(id):
	...
	if id != None:
		...
		if request.method == 'POST':
			...
			new_book = book_manager.edit(title=title, quantity=quantity, available=available, description=description, edition=edition, id=book_id)
